...
class GameView(context: Context) : View(context) {
    ...
    private fun drawCoin(c:Canvas,i:Item){if(i.used)return;val x=i.r.x-camX;if(x>W||x+i.r.w<0)return;val bob=sin(time*6+i.r.x)*3;paint.color=Color.argb(76,255,210,63);c.drawCircle(x+11,i.r.y+11+bob,15,paint);paint.color=Color.rgb(255,210,63);c.drawCircle(x+11,i.r.y+11+bob,11,paint);paint.color=Color.rgb(107,74,18);paint.textSize=12f;paint.textAlign=Paint.Align.CENTER;c.drawText("👍",x+11,i.r.y+15+bob,paint)}
    private fun drawBell(c:Canvas,i:Item){if(i.used)return;val x=i.r.x-camX;if(x>W||x+i.r.w<0)return;val bob=sin(time*5+i.r.x)*4;paint.color=Color.argb(76,255,61,110);c.drawCircle(x+13,i.r.y+15+bob,18,paint);paint.color=Color.WHITE;paint.textSize=24f;paint.textAlign=Paint.Align.CENTER;c.drawText("🔔",x+13,i.r.y+30+bob,paint)}
    private fun drawBush(c:Canvas,i:Item){val x=i.r.x-camX;if(x>W||x+i.r.w<0)return;paint.color=if(i.used)Color.rgb(47,106,47)else Color.rgb(36,90,36);c.drawCircle(x+15,i.r.y+30,18,paint);c.drawCircle(x+30,i.r.y+25,20,paint);c.drawCircle(x+42,i.r.y+33,15,paint)}
